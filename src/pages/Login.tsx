...
const loginSchema = z.object({
  email: z.string().trim().min(1, { message: "Email is required." }).email({
    message: "Please enter a valid email address (e.g. admin@gmail.com).",
  }),
  password: z
    .string()
        ...
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-offYellow">
                Email Address
              </label>
              <input
                type="email"
          ...
          <div className="rounded border border-amber-900 bg-amber-950/20 p-3 text-xs text-amber-400">
            🔑 **Developer Notice**: The form is pre-filled with the default
            admin account seeded during database setup.
          </div>

          <button
            type="submit"
            disabled={isLoading}
            className="w-full rounded bg-yellow py-2.5 px-4 text-center text-sm font-semibold text-[var(--primary-foreground)] shadow transition hover:opacity-95 disabled:opacity-50"
          >
